...
class ControllerExtensionPaymentArsenalpay extends Controller {
	...
	public function index() {
		...
		$widget_id   = $this->config->get('payment_arsenalpay_widget_id');
		$widget_key  = $this->config->get('payment_arsenalpay_widget_key');
	...
	public function ap_callback() {
		...
		$IP_ALLOW = trim($this->config->get('payment_arsenalpay_ip'));
		...
		if ($order === false || $order['order_id'] != $ap_order_id) {
			...
			$this->model_checkout_order->addOrderHistory($ap_order_id, $this->config->get('payment_arsenalpay_failed_status_id'), $comment, true);
		...
		$KEY = $this->config->get('payment_arsenalpay_callback_key');
	...
	private function callback_cancel($order) {
		$required_statuses = array(
			$this->config->get('payment_arsenalpay_holden_status_id'),
			$this->config->get('payment_arsenalpay_checked_status_id'),
		);
		...
		$this->log($comment);
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_canceled_status_id'), $comment, true);
		$this->exitf('OK');
	}

	private function callback_check($order) {
		$rejected_statuses = array(
			$this->config->get('payment_arsenalpay_canceled_status_id'),
			$this->config->get('payment_arsenalpay_reversed_status_id'),
			$this->config->get('payment_arsenalpay_refunded_status_id'),
			$this->config->get('payment_arsenalpay_completed_status_id'),
		);
		...
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_checked_status_id'), $comment, true);
	...
	private function prepare_fiscal($order) {
		...
		$RUB        = $this->config->get('payment_arsenalpay_currency_code');
		...
		if ($shipping > 0) {
			...
			$tax_name = $this->config->get("payment_arsenalpay_shipment_tax_rate");
			if (isset($tax_name) && strlen($tax_name) > 0) {
	...
	private function get_arsenalpay_tax_by_shop_tax_id($shop_tax_id) {
		$default_tax   = $this->config->get("payment_arsenalpay_default_tax_rate");
		$tax_rates_map = $this->config->get("payment_arsenalpay_tax_rates_map");
		$tax           = $default_tax;
	...
	private function callback_payment($order) {
		...
		elseif ($this->callback['MERCH_TYPE'] == 1 && $this->get_total($order) >= $this->callback['AMOUNT'] && $this->get_total($order) == $this->callback['AMOUNT_FULL']) {
			$currency_code = $this->config->get('payment_arsenalpay_currency_code');
		...
		$this->log($comment);
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_completed_status_id'), $comment, true);
		$this->exitf('OK');
	}
	...
	private function callback_hold($order) {
		...
		$this->log($comment);
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_holden_status_id'), $comment, true);
		$this->exitf('OK');
	}
	...
	private function callback_refund($order) {
		...
		$this->log($comment);
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_refunded_status_id'), $comment, true);
		$this->exitf('OK');
	}

	private function callback_reverse($order) {
		if ($order['order_status_id'] != $this->config->get('payment_arsenalpay_completed_status_id')) {
		...
		$this->log($comment);
		$this->model_checkout_order->addOrderHistory($order['order_id'], $this->config->get('payment_arsenalpay_reversed_status_id'), $comment, true);
		$this->exitf('OK');
	}
...
//		$from = $order['currency_code'];
		$to = $this->config->get('payment_arsenalpay_currency_code');
...
//		$amount = $this->currency->format( $amount, $order['currency_code'], false, false );
	...
	private function log($message) {
		if ($this->config->get('payment_arsenalpay_debug')) {
